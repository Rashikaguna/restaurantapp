DELIMITER $$                                                                              
CREATE PROCEDURE sub_cancel(IN mealnumber INT,IN foodparam INT, IN i_order INT,IN i_qty INT)
BEGIN
 IF (mealnumber IN(SELECT id FROM foodtype WHERE  from_time<=CURTIME() AND to_time>=CURTIME())) THEN
   ...
   UPDATE food_stocks SET remaining= remaining+i_qty WHERE foodid=foodparam;
ELSE
...
DELIMITER $$
CREATE PROCEDURE cancel_per_food_quantity(IN i_orderparam INT,IN quantityparam INT)
BEGIN
...
CALL sub_cancel(foodnumber,foodparam,i_orderparam,quantityparam);
ELSE
